...
output_layers = [layer_names[i - 1] for i in net.getUnconnectedOutLayers()]  # this gets the final layer(s)
colors = np.random.uniform(0, 255, size=(len(classes), 3))

# Loading Video
cap = cv2.VideoCapture("WIN_20211201_13_36_29_Pro.mp4")
...
while True:
    ...
    # blob is essentially a accessible collection of parameters of the image, height,width
    ...
    for out in outs:  # for one output/node in the net
        for detection in out:  # for one detection of output/node
            scores = detection[5:]  # get everything that's not center_x,center_y, w, h, x, y
